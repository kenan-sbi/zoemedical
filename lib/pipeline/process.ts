// Document pipeline: OCR text -> Reader extraction -> persist with provenance -> demographics ->
// dating -> verify. Lifted VERBATIM from the BullMQ worker so the extraction/analysis contract is
// unchanged; it can now run either in the worker (local dev) or inline via waitUntil (serverless).
import { prisma } from '../db';
import '../llm'; // side-effect: registers the LLM providers (gemini) before extraction runs
import { readDocument } from '../llm/reader';
import { verifyFacts, verifierEnabled } from '../llm/verifier';
import { persistFacts } from './persist';
...
export async function processDocument(documentId: string) {
  ...
  try {
    ...
    // Mark the job FAILED so the UI can show it, then rethrow (BullMQ retries; inline caller logs).
